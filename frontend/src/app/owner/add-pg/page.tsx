  ...
  return (
    <div className="min-h-screen bg-slate-50/45 pb-16 pt-6 sm:py-12 antialiased">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* ============ HERO BANNER SECTION ============ */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-white rounded-3xl p-6 md:p-12 border border-slate-100 shadow-sm mb-12">
          <div>
            <span className="text-xs font-bold uppercase tracking-widest text-indigo-600 mb-2 block">LIST YOUR PG</span>
            <h1 className="text-3xl sm:text-4xl font-black text-slate-950 tracking-tight leading-tight">
              List Your PG, <br />
              <span className="text-pink-600">Reach Thousands of Students</span>
            </h1>
            <p className="mt-4 text-slate-600 text-sm leading-relaxed max-w-sm">
              Complete our interactive 6-stage verification wizard to showcase your property directly to authenticated student leads.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-8">
            ...
            {STEPS.map((step) => (
              <div key={step.id} className="flex flex-col items-center gap-2 bg-white px-4 relative z-10">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border-2 transition-all ${
                  step.id === currentStep ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 
                  step.id < currentStep ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-white border-slate-200 text-slate-400'
                }`}>
                  {step.id < currentStep ? '✓' : step.id}
                </div>
                <span className={`text-xs font-extrabold tracking-tight ${step.id === currentStep ? 'text-indigo-600' : 'text-slate-400'}`}>
                  {step.label}
                </span>
              </div>
            ))}
          ...
          <div className="md:hidden flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              <span className="w-7 h-7 bg-indigo-600 rounded-full text-white font-bold text-xs flex items-center justify-center">{currentStep}</span>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
            <div className="border-b border-slate-100 pb-5 mb-6 flex justify-between items-center">
              ...
              {currentStep > 1 && (
                <button type="button" onClick={handleSaveDraft} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 bg-indigo-50 px-2.5 py-1.5 rounded-lg transition-all" title="Save current progress as draft">
            ...
            <form onSubmit={currentStep === 6 ? handleSubmit : (e) => e.preventDefault()} className="space-y-6">
                ...
                <div className="space-y-5 animate-fadeIn">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input required type="text" name="pgName" value={form.pgName} onChange={handleInputChange} placeholder="e.g. Stanza Living Boston House" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none focus:bg-white focus:border-indigo-600 transition-all" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-black text-slate-800 tracking-wide">PG Category <span className="text-rose-500">*</span></label>
                      <select required name="category" value={form.category} onChange={handleInputChange} className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none focus:bg-white focus:border-indigo-600 transition-all cursor-pointer">
                ...
                <div className="grid grid-cols-2 gap-4 animate-fadeIn">
                  {[
                    ...
                    <label key={amenity.id} className={`flex items-center gap-3 p-4 border rounded-2xl cursor-pointer transition-all ${
                      form[amenity.id as keyof typeof form] ? 'border-indigo-600 bg-indigo-50/20' : 'border-slate-100 bg-slate-50/30 hover:bg-slate-50'
                    }`}>
                      <input type="checkbox" checked={!!form[amenity.id as keyof typeof form]} onChange={(e) => handleCheckboxChange(amenity.id, e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4" />
              ...
              {currentStep === 3 && (
                <div className="space-y-4 animate-fadeIn">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input required type="text" name="address" value={form.address} onChange={handleInputChange} placeholder="House No, Building name, Street name" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none focus:bg-white" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <input required type="text" name="city" value={form.city} onChange={handleInputChange} placeholder="City" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none" />
                    <input required type="text" name="state" value={form.state} onChange={handleInputChange} placeholder="State" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none" />
                    <input required type="text" name="pincode" value={form.pincode} onChange={handleInputChange} placeholder="Pincode" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-black text-slate-800 tracking-wide">Nearby Landmark <span className="text-rose-500">*</span></label>
                    <input required type="text" name="landmark" value={form.landmark} onChange={handleInputChange} placeholder="e.g. Near Metro Station Gate No. 2" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none" />
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4 animate-fadeIn">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-2">
                      {previews.map((src, index) => (
                        <div key={index} className="relative aspect-square border border-slate-100 rounded-xl overflow-hidden group shadow-sm">
                          ...
                          <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 p-1.5 bg-rose-600 rounded-lg text-white opacity-90 hover:opacity-100 transition-opacity">
                  ...
                  <div className="bg-slate-50 p-3 rounded-xl flex items-center gap-2">
                    ...
                    <p className="text-[11px] font-bold text-slate-600">
                      Current Count: <span className="text-indigo-600 font-extrabold">{previews.length}</span> images loaded. 
                ...
                <div className="space-y-4 animate-fadeIn">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input required type="number" name="rent" value={form.rent} onChange={handleInputChange} placeholder="e.g. 8500" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none focus:bg-white" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-black text-slate-800 tracking-wide">Security Deposit (₹) <span className="text-rose-500">*</span></label>
                      <input required type="number" name="securityDeposit" value={form.securityDeposit} onChange={handleInputChange} placeholder="e.g. 15000" className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none focus:bg-white" />
                    </div>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select name="noticePeriod" value={form.noticePeriod} onChange={handleInputChange} className="w-full text-xs font-semibold px-4 py-3 border border-slate-200 rounded-xl bg-slate-50/50 outline-none cursor-pointer">
              ...
              {currentStep === 6 && (
                <div className="space-y-6 animate-fadeIn">
                  <div className="bg-indigo-50/40 border border-indigo-100/70 rounded-2xl p-4 text-xs font-bold text-indigo-950 flex gap-2.5 items-start">
                    <CheckCircle2 className="w-4 h-4 text-indigo-600 shrink-0 mt-0.5" />
                    <div>
                      <p>Data Verification Auditing Mode Active</p>
                      <p className="text-[10px] text-indigo-600 font-medium mt-0.5">Please scan through your compiled dataset below before publishing onto the production network.</p>
                    </div>
                  </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-3 pt-6 border-t border-slate-100">
                {currentStep > 1 ? (
                  <div className="flex w-full sm:w-auto gap-2">
                    <button type="button" onClick={prevStep} className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 border-2 border-slate-200 text-slate-600 font-extrabold px-5 py-3 rounded-xl hover:bg-slate-50 text-xs tracking-wide shadow-sm" >
                      <ArrowLeft className="w-4 h-4" /> Back
                    </button>
                    <button type="button" onClick={handleSaveDraft} className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 border border-dashed border-indigo-200 bg-indigo-50/30 text-indigo-600 font-bold px-4 py-3 rounded-xl hover:bg-indigo-50 text-xs" >
                      Save Draft
                    </button>
                  </div>
                ) : (
                  <button type="button" onClick={handleSaveDraft} className="w-full sm:w-auto text-center border-2 border-slate-200 text-slate-600 font-extrabold px-5 py-3 rounded-xl hover:bg-slate-50 text-xs tracking-wide shadow-sm" >
                    Cancel & Save Draft
                  </button>
                )}

                {currentStep < 6 ? (
                  <button type="button" onClick={nextStep} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-indigo-600 text-white font-extrabold px-6 py-3 rounded-xl hover:bg-indigo-700 text-xs tracking-wide shadow-sm" >
                    Continue <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
                  <button type="submit" disabled={loading} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-emerald-600 text-white font-extrabold px-8 py-3 rounded-xl hover:bg-emerald-700 disabled:opacity-60 text-xs tracking-wide shadow-md active:scale-97" >
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { icon: <ShieldCheck className="w-4 h-4" />, color: 'bg-indigo-50 text-indigo-600 border-indigo-100', title: 'Trusted Platform', desc: 'We verify every listing to maintain quality and trust.' },
                  { icon: <MessageSquare className="w-4 h-4" />, color: 'bg-pink-50 text-pink-600 border-pink-100', title: 'Direct Contact', desc: 'Students can contact you directly via call or WhatsApp.' },
                  { icon: <Tv className="w-4 h-4" />, color: 'bg-purple-50 text-purple-600 border-purple-100', title: 'Grow Your Business', desc: 'Get more inquiries and increase your bookings.' },
                ].map((item, i) => (
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
              ...
              <Link href={ROUTES.SUPPORT} className="w-full inline-flex items-center justify-center gap-2 border-2 border-indigo-600 text-indigo-700 font-extrabold px-4 py-2.5 rounded-xl hover:bg-indigo-600 hover:text-white transition-all duration-200 text-xs shadow-sm" >
